...
const Grocery = () =>{
  const [groceryData, setGroceryData] = useState(InitialGrocery);
  const groceriesHandler = useCallback( async (userData) => {
    let loginUrl, payload;

        loginUrl = '/api/grocery';
        payload = {
  ...
  useEffect(() =>{
    ...
    const tableContent = groceryData.map(item => {
        return (
            <tr>
                <td>{item.itemDesc}</td>
                <td>{item.itemQty}</td>
                ...
                <td><img class="card-img-top" src={item.itemImg} alt="Card image cap"/></td>
                <td>{item.orderId}</td>
            </tr>
        )
    })
